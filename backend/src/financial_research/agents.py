...
COMMON_RULES = (
    ...
    "明确区分事实与推断：事实类由 evidence_ids 字段指向证据，推断类说明依据。"
    "每条 finding 必须引用至少一个提供的 evidence_id。"
    # 正文是给用户读的：market-3f2a… 这种标识只有程序认得，写进正文等于什么都没说。
    # 引用关系由 evidence_ids 字段承载，正文里改用证据的 source/title 指代来源。
    "正文面向用户，不要出现 evidence_id 这类机器标识；要指代来源时用证据的 source 或 title 文字。"
...
AGENT_SPECS: dict[str, AgentSpec] = {
    ...
    "market": AgentSpec(
        ...
        description=(
            ...
            "上下文已提供全样本的逐日事实（区间高低点及日期、最大单日波动及当日开高低收、跳空、"
            "零成交日、量比、价量相关、成交额覆盖天数）与最近一段逐日明细，直接使用这些字段，"
            "不要把它们列为待补充的数据。"
            "除权归因（action_attribution）已把最大单日波动与除权日对齐并给出当日实测影响，"
            "corporate_actions 是区间内的分红送转公告；cross_check 是独立第二源的逐日比对结论。"
            "这三项都已给出，不要把它们列为待补充的数据。"
            "注意 amount_available 为 false 表示该行情源不提供成交额，这是数据源边界，不是缺漏。"
        ),
        schema=AgentFinding,
    ...
    "technical": AgentSpec(
        ...
        role="技术分析师",
        description=(
            "你是技术分析师。基于给定的确定性指标（均线、RSI、波动率、最大回撤、量比、价量相关、"
            "区间高低点、最大单日波动）解读过去的趋势、动量与量价配合。"
            "所有数值由程序计算，你只做解读，不得重新计算或修改数值，也不要索要已给出的指标。"
    ...
    "macro": AgentSpec(
        ...
        role="宏观分析师",
        description=(
            "你是宏观分析师。解释给定的利率与信用环境通过融资成本与折现率影响估值的传导路径。"
            "上下文按指标给出：国债收益率、CPI/PPI 同比、M2/M1 同比、存款准备金率、制造业 PMI，"
            "每项附最近观测与全窗口的条数、首末日期、极值（stats）。"
            "统计类指标按月发布且有发布时滞，最新一期落后截止日一到两个月是口径使然，"
            "不要把它列为待补充的数据。单一指标不能确定价格方向，方向与强度依赖企业盈利与市场预期。"
        ),
        schema=AgentFinding,
    ),
    "risk": AgentSpec(
        ...
        description=(
            ...
            "证据池里若有 kind 为 fundamental 的证据，说明主要财务指标本轮已经取到，"
            "不要再把「缺少财务数据」列为证据缺口；design_limits 里列出的是设计边界而非缺口。"
    ...
    "arbiter": AgentSpec(
        ...
        role="首席仲裁",
        description=(
            "你是首席研究仲裁。当结论方向出现冲突时，依据证据强度做出裁决："
            "说明冲突是什么、采纳哪一方的判断、依据是什么。"
            "冲突有两类，裁决依据不同："
            "一是价格趋势与供应商新闻情绪标签相反，此时价格与成交属于一手观测、"
            "情绪标签属于未经原文核验的二手聚合，权重不同；"
            "二是均线排列与 RSI 背离（趋势跟随读法与均值回归读法在同一段价格上给出相反信号），"
            "此时两边都源自同一段价格序列，不能声称某一方证据更强，只说明当前方向的确定性有限。"
            "你只会在确实存在方向冲突时被调用，因此切勿返回空裁决。"
        ),
        schema=Arbitration,
        # optional=True 在此表示「由冲突信号触发」，与 news/macro 的「可由计划启停」含义不同：
        # arbiter 绝不能被传给 enabled() 判断，它只由 conflict_findings() 决定是否运行。
        optional=True,
    ),
    "report": AgentSpec(
        key="report",
        ...
        description=(
            ...
            "必须说明证据覆盖范围与不确定性，不得引入未提供的信息。"
            "evidence 中 kind 为 fundamental 的条目是主要财务指标（数值取自财报接口，"
            "口径与覆盖区间写在它的 note 里），可以引用；design_limits 是设计边界而非缺口，"
            "不要把其中的项目写进 uncertainties。"
