...
foreach ($port in @(8877, 3000)) {
    if (Get-NetTCPConnection -State Listen -LocalPort $port -ErrorAction SilentlyContinue) {
        throw "Port $port is already in use. If Atlas is running, open http://localhost:3000."
    }
}
$env:NEXT_TELEMETRY_DISABLED = '1'
$apiProcess = Start-Process -FilePath $pythonPath -ArgumentList @('-m', 'uvicorn', 'financial_research.api:app', '--host', '127.0.0.1', '--port', '8877') -WorkingDirectory $projectRoot -WindowStyle Hidden -PassThru -RedirectStandardOutput (Join-Path $runPath 'api.log') -RedirectStandardError (Join-Path $runPath 'api-error.log')
...
for ($attempt = 0; $attempt -lt 40; $attempt++) {
    try {
        $apiHealth = Invoke-RestMethod -Uri 'http://127.0.0.1:8877/api/health' -TimeoutSec 2
...
Write-Host 'API documentation: http://localhost:8877/docs'
