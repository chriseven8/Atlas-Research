...
const AGENTS = [
  ...
  ["macro", "宏观研究", "观察利率环境"], ["risk", "风险审查", "识别缺口与分歧"],
  ["arbiter", "首席仲裁", "裁决结论冲突"], ["report", "研究报告", "汇总结论与证据"],
];
const AGENT_NAME: Record<string, string> = Object.fromEntries(AGENTS.map(([key, name]) => [key, name]));
const PIPELINE = ["manager", "market", "technical", "news", "macro", "risk", "arbiter", "report"];
...
function Collaboration({ report }: { report: Report }) {
...
export default function Home() {
  ...
  const executedPath = detail
    ? PIPELINE.filter(key => detail.agents.some(a => a.name === key && a.status && a.status !== "skipped")).map(key => AGENT_NAME[key]).join(" → ")
    : "";
  ...
  return <div className="app-shell">
    ...
    <div className="main-shell"><header className="topbar"><div><span>工作空间</span><ChevronRight size={13} /><strong>{view === "history" ? "研究记录" : view === "settings" ? "数据与模型" : "研究工作台"}</strong></div><div className="topbar-right"><span className="local-pill"><span />{config ? "服务已连接" : "连接服务中"}</span><button className="icon-button" onClick={() => setHelp(true)} aria-label="使用帮助"><CircleHelp size={18} /></button></div></header>
      ...
      <section className="panel team-panel"><div className="panel-head"><h2><Layers3 size={17} /> 研究协作流程</h2>{detail ? <StatusBadge status={detail.status} /> : <span className="badge idle"><span className="badge-dot" />准备就绪</span>}</div><div className="team-intro"><h3>{active ? "正在连接线索，形成研究。" : report ? "研究已就绪，证据已归档。" : "一个问题，八个研究视角。"}</h3><p>{detail?.error || (detail?.status === "cancelled" ? "任务已取消。你可以修改研究条件，重新发起。" : "每个角色独立处理任务，再由风险审查与报告角色汇合结果。")}</p></div><div className="agent-grid">{AGENTS.map(([key, name, desc], i) => { const agent = detail?.agents.find(a => a.name === key); return <div className={`agent-card ${agent?.status || "idle"}`} key={key}><div className="agent-top"><span className="agent-number">0{i + 1}</span>{agent?.status === "running" ? <LoaderCircle size={15} className="spin" /> : agent?.status === "completed" ? <CheckCircle2 size={15} /> : agent?.status === "partial" ? <CircleHelp size={15} /> : agent?.status === "failed" ? <X size={15} /> : <Circle size={13} />}</div><h4>{name}</h4><p>{desc}</p>{agent?.status === "skipped" && agent.output?.reason && <p className="skip-reason">{agent.output.reason}</p>}<small>{agent ? LABELS[agent.status] : "待命"}{agent?.duration_ms != null ? ` · ${agent.duration_ms < 1000 ? `${agent.duration_ms}ms` : `${(agent.duration_ms / 1000).toFixed(1)}s`}` : ""}</small></div>; })}<div className="agent-card outcome"><FileText size={20} /><strong>可追溯报告</strong><p>观点 · 风险 · 来源</p><small>Markdown / JSON</small></div></div><div className="team-footer"><span><ShieldCheck size={15} />{executedPath || "数据校验 → 并行分析 → 风险审查 → 报告"}</span>{active && <button onClick={cancel}><Square size={12} /> 取消任务</button>}</div></section></div>
